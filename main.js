...
async function registerpage(page) {
    ...
    if (settings.mode_record) {
        ...
        if(settings.enable_scripts)
        {
            ...
            {
                console.error("replay_record_functions crashed with error",err)
            }
        }
    }
    ...
    try
    {
        ...
        )
        {
            try
            {
                ...
                console.log("request interception failed with error",err)
                process.exit();
            }
